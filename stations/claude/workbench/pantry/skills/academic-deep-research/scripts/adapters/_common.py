...
def parse_family_year_key(authors: list[dict[str, str]] | None, year: int | None) -> str:
    """Build the {first_family_lower}{year} join key used to match a
    bibliography block against a row in the verification table.

    Returns an empty string if no family is found (caller treats that as
    a parse failure and rejects the entry). Year is appended as a plain
    integer so 2025 and "2025" produce the same key.
    """
    if not authors:
        return ""
    first = authors[0]
    family = (first.get("family") or first.get("literal") or "").strip()
    if not family:
        return ""
    family_clean = _NON_ALNUM.sub("", family).lower()
    if not family_clean:
        return ""
    if year is None:
        return family_clean
    return f"{family_clean}{int(year)}"
