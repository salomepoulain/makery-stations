...
> **Routing discipline (v3.9.2):** see `docs/SKILL_GUIDE.md` "Routing Discipline (v3.9.2)" + `shared/references/intent_clarification_protocol.md` for cross-skill routing rules. This skill assumes routing has already settled — ambiguous cross-phase materials should have been clarified upstream.
...
**Execution:**
Phase artifacts go to `docs/academic-research/` in the project root (not the skill directory). See `docs/SKILL_GUIDE.md` for the full convention.
...
After Phase 2 completes, run `scripts/adapters/ars_pipeline.py` against the bibliography + verification markdown to emit a literature corpus in machine-readable form (Zotero CSL-JSON, BibTeX, citation graph). See `docs/SKILL_GUIDE.md` "Corpus export" and `scripts/adapters/README.md` for the contract.
...
Routing into Mode B requires explicit user signal — `/ars-<mode>` slash command or `[direct-mode]` prefix. Ambiguous cross-phase input defaults to clarification per `docs/SKILL_GUIDE.md` Routing Discipline + `shared/references/intent_clarification_protocol.md`.
...
| `examples/handoff_to_paper.md` | academic-deep-research full mode handoff to academic-paper |
...
academic-deep-research + academic-paper          -> Full research-to-publication pipeline
academic-deep-research (socratic) + academic-paper (plan) -> Guided research + paper planning
academic-deep-research (systematic-review) + academic-paper -> PRISMA systematic review paper
```
