...
2. Trigger handoff protocol (defined in each skill's SKILL.md):
   - Stage 1  --> 2: academic-deep-research handoff (RQ Brief + Methodology Blueprint + Bibliography + Synthesis)
...
academic-pipeline is the orchestrator skill that coordinates the full ARS pipeline across 10 stages (delegating to academic-deep-research, academic-paper, academic-paper-reviewer). Two invocation modes:
...
| Stage 1 | academic-deep-research not converging | Suggest mode switch (socratic -> full) or narrow scope |
...
Stage 1: academic-deep-research
...
| `graphlookup` | Not dispatched by the pipeline — run manually after Stage 1 to expand the corpus by crawling real citation edges outward from the seed papers. See `graphlookup/SKILL.md`. |
...
| Dependent Skills | academic-deep-research v2.0+, academic-paper v2.0+, academic-paper-reviewer v1.1+ |
